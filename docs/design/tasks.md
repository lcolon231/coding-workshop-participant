...
- [x] **T83** Incident list — filters, pagination, empty/loading/error states. *(`pages/IncidentsPage.jsx`: filters and page live in the URL, search debounced, table above `md` and stacked rows below; 8 tests)*
...
- [x] **T131** Frontend coverage pass. *(The untested seams, not the easy lines: `AuthProvider` had no test at all (session load through `/me`, a 401 clearing the session, another failure kept as an error with retry, sign-out revoking best-effort, a session appearing later and surviving a rotation); `App`'s loading frame and account-error screen; the `session` store including storage that throws; `useWidth` against a fake `ResizeObserver`; the service-worker registration; `PieChart` hover, focus, legend and the fold into "Other"; every `facilities` service wrapper table-driven; category edit and delete from the row menu. 88.6 → 92.7 % statements, 81.6 → 84.3 % branches, 85.0 → 90.9 % functions; floors raised to 90 / 80 / 88. 51 new tests)*
- [x] **T127** Response targets live on every incident, not only in the reports. *(`acme_core/reporting.py` grades one incident against its D8 target: `due_at` and `sla_state` (`on_track` / `at_risk` at a quarter of the target left / `breached`; `met` / `missed` once finished) are computed fields on `IncidentOut`, so every list row and detail carries them without a column; `overdue=true|false` on the list and `due_at` in the sort allowlist do the same arithmetic in SQL, by the database clock. Frontend: `SlaChip` (a countdown while open, the verdict once finished, the deadline as tooltip) in a Target column of the list, on the cards, in the detail header and in the CSV; an "Overdue" switch and "Due soonest" sort on the list; an "Overdue now" section at the top of the admin overview, worst first, linking to the filtered list. 7 unit + 3 integration tests, 9 frontend tests)*
