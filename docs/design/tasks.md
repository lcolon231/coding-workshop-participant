...
- [x] **T124** Dashboard: buildings, engineers and every incident. *(I18 `/reports/buildings` and I19 `/reports/engineers`, one outer-join `GROUP BY` each so a quiet building or an idle engineer shows as zero rather than vanishing; `created_from`/`created_to` on the incident list. `pages/ReportsPage.jsx` gains three sections under the same range and building: buildings ranked by incidents, still open and critical; engineers ranked by completed and by open workload with a table of assigned / open / completed / mean time to resolve; and a paged "All incidents" table with a "Download CSV" that walks every page of the range into one RFC 4180 file. 6 backend tests, 9 frontend tests)*
